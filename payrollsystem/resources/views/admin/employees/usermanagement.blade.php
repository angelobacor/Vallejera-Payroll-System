...
@if (session('info'))
    <div id="flash-message-2" class="alert alert-info">
        {{ session('info') }}
    </div>
@endif
...
<div class="h-full ml-14 mt-14 mb-5 md:ml-64">
    <div class="flex items-center justify-between mt-5 ml-2">
        <h1 class="text-2xl flex-grow">User Management</h1>
    ...
    <div class="flex justify-end items-center m-2" style="margin-top:-20px;">
        <form action="{{ route('employee_update_search') }}" method="GET" enctype="multipart/form-data" class="flex items-center space-x-2">
            @csrf
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">System Access</th>
            ...
            @if($employees->count() > 0)
                @foreach($employees as $employee)
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">{{$employee->designation->designation_name}}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{$employee->department->department_name}}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{ ucfirst($employee->role) }}</td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center justify-between">
                                ...
                                <div class="relative flex items-center space-x-3">
                                    ...
                                    <button class="focus:outline-none" onclick="openUserModal('{{ $employee->id }}')">
                                    ...
                                    <button class="focus:outline-none" onclick="openDedUserModal('{{$employee->id}}')">
...
<script>
    ...
    function openUserModal(id) {
        $.get('/admin/user-manage/' + id + '/modal', function (html) {
            // alert(html)
            $('#modalBody').html(html);
            // show by removing 'hidden'
            $('#updateemployee').removeClass('hidden');
        });
    }

    function closeUpdEmployee() {
        $('#updateemployee').addClass('hidden');
    }

    function openDedUserModal(id) {
        $.get('/admin/user-manage/' + id + '/deduction-modal', function (html) {
            // alert(html)
            $('#modalBody').html(html);
            // show by removing 'hidden'
            $('#deductionEmp').removeClass('hidden');
        });
    }

    function closeDedEmployee() {
        $('#deductionEmp').addClass('hidden');
    }
