...
<div class="h-full ml-14 mt-14 mb-5 md:ml-64">
    <div class="flex items-center justify-between mt-5 ml-2">
        <h1 class="text-2xl flex-grow">Employee Deductions</h1>
    ...
    <div class="flex justify-end items-center m-2" style="margin-top:-20px;">
        <form action="{{ route('hr_employee_update_search') }}" method="GET" enctype="multipart/form-data" class="flex items-center space-x-2">
            @csrf
...
<script>
    ...
    function openDedUserModal(id) {
        $.get('/payroll-officer/user-manage/' + id + '/deduction-modal', function (html) {
            // alert(html)
            $('#modalBody').html(html);
