...
<div id="deductionEmp" class="fixed inset-0 bg-gray-800 bg-opacity-60 flex items-center justify-center hidden z-50">
    <div class="bg-white p-4 rounded-lg shadow-lg relative w-full max-w-2xl">
        ...
        <h2 class="text-2xl font-semibold mb-4 text-gray-800">Deductions</h2>
                ...
                <tbody>
                    @forelse($deductions as $ded)
                        <tr class="border-b">
                            ...
                            <td class="px-3 py-2 ">
                                <a href="{{ route('hr_delete_deduction', $ded->id) }}" onclick="return confirm('Are you sure you want to delete this deduction item?');"
        ...
        <h2 class="text-center fw-bold">Add Section</h2>
        <hr>
        <br>
        <!-- FORM -->
        <form method="POST" action="{{ route('hr_add_deduction') }}">
            @csrf
