...
class AdminController extends Controller
{
    ...
    /* Get total hours per month without SUNDAY
    $totalHours = 0;
    ...
    */
    ...
    $probations = User::where('employment_id', '=', '2')->get();
    $regulars = User::where('employment_id', '=', '1')->get();
    $terminated = User::where('employment_id', '=', '3')->get();
    ...
    $hours = 206;
    $balanceHours = $hours - $totalActiveHours;
        return view('admin.home',compact('departments', 'employees', 'hours','totalActiveHours', 'getAttendance', 'balanceHours', 'payruns', 'probations', 'regulars', 'terminated', 'user'));
    }
    ...
    public function searchpayslip(Request $request) {
        ...
        return view('admin.payroll.searchpayslip', compact('employee_payslips', 'user'));
    }
    ...
    public function filterDateAttendanceAdmin(Request $request)
    {
        ...
        return view('admin.employees.allattendance', compact('getAttendance', 'user','departments'));
    }
    ...
    public function recalculate($id) {
        ...
        return view('admin.payroll.recalculatepayslip', compact('payslip', 'user', 'basicsalary'));
    }
    ...
    public function leaver(Request $request){
        ...
        $totalLeaves = Leave::where('user_id', '=', $request->employee)->where('status', '=', 'Approved')
        ...
        if($totalLeaves->count() <= 19){
            if ($request->leave_type == 'Leave'){
                if ($request->is_paid && !$paid_leave_count >= 5){
            ...
            return redirect()->back()->with('message', 'Employee have reached the maximum number of leaves for this Year');
        }
    ...
    public function punchout(Request $request){
        ...
        $daily = ($salary/160)*8; //208 represents month total hours I just inputed 206 on the hours below dont be confused
    ...
    public function payrun(Request $request){
        ...
        return redirect()->route('payrunview');
    }

    public function paymentIncrease(Request $request){
        date_default_timezone_set('Asia/Manila');

        if($request->custompay){
            HolidayPaymentIncrease::create([
                'amount' => $request->custompay
            ]);
        }else{
            HolidayPaymentIncrease::create([
    ...
    {
        $attendances = Leave::select(['user_id', 'start_date', 'end_date', 'leave_type', 'activity', 'duration', 'status'])->get();
        ...
        $sheet->setCellValue('D1', 'Activity');
        $sheet->setCellValue('E1', 'Duration');
        $sheet->setCellValue('F1', 'Status');
    
        foreach ($attendances as $rowNumber => $attendance) {
            $sheet->setCellValue('A' . ($rowNumber + 2), $attendance->employee->name);
            $sheet->setCellValue('B' . ($rowNumber + 2), $attendance->start_date . ' - ' . $attendance->end_date);
            $sheet->setCellValue('C' . ($rowNumber + 2), $attendance->leave_type);
            $sheet->setCellValue('D' . ($rowNumber + 2), $attendance->activity);
            $sheet->setCellValue('E' . ($rowNumber + 2), $attendance->duration);
            $sheet->setCellValue('F' . ($rowNumber + 2), $attendance->status);
        }
    
        $fileName = 'Employee Leave Requests' . Auth::user()->id . '.xlsx';
    ...
    public function import(Request $request)
    {
        ...
        return redirect('/admin/all-employee-attendance')->with('success', 'Attendance imported successfully!');
    }
    ...
    return redirect()->route('adminpayslip', $payrun->id)->with('employee_payslips', 'user');
    }
    ...
    public function upload(Request $request)
    {
        ...
        if ($file->isValid()) {
            ...
            // Move the file to the 'uploads' directory on the 'public' disk
    ...
    public function attendance_filter(Request $request)
    {
        ...
        return view('admin.jobdesk.attendance', compact('getAttendance', 'user', 'totalActiveHours', 'monthSearch'));
    ...
    public function downloadDocument($id)
{
    ...
    if ($document) {
        $filePath = public_path($document->attachment);
        $fileName = basename($document->attachment);
    ...
    public function manualattendance(Request $request){
        ...
        if($request->morning_punchin){
            // Ensure that the input times are Carbon instances
            $morningPunchIn = Carbon::parse($request->morning_punchin);
            $morningPunchOut = Carbon::parse($request->morning_punchout);
            
            // Calculate total hours worked
            $total_hours = $morningPunchIn->diffInHours($morningPunchOut);

            // Fetch employee details
            $employee = User::find($request->employee);
            $salary = (int)str_replace(',', '', $employee->salary);
            $amountbasedonhour = ($salary / 20); // 208 represents month total hours

            // Determine behavior based on punch-in time
            if ($morningPunchIn->format('H:i') < '08:00') {
                $behavior = 'Early';
            } elseif ($morningPunchIn->format('H:i') >= '08:00') {
                $behavior = 'Regular';
            } elseif ($morningPunchIn->format('H:i') > '08:30') {
                $behavior = 'Late';
            } else {
                // Handle unexpected cases if necessary
                return response()->json(['error' => 'Invalid punch-in time'], 400);
            }
            ...
            Attendance::create([
                'user_id' => $request->employee,
                'date' => $request->date,
                'punched_in' => $morningPunchIn,
                'punched_out' => $morningPunchOut,
                'behavior' => $behavior,
                'type' => 'Manual',
                'total_hours' => $total_hours,
                'entry' => 'Single',
                'status' => '',
                'amount' => round($amountbasedonhour, 2)
            ]);

            if($request->afternoon_punchin){
                // Ensure that the input times are Carbon instances
                $afternoonPunchIn = Carbon::parse($request->afternoon_punchin);
                $afternoonPunchOut = Carbon::parse($request->afternoon_punchout);
                
                // Calculate total hours worked
                $total_hours = $afternoonPunchIn->diffInHours($afternoonPunchOut);
                ...
                // Determine behavior based on punch-in time
                if ($afternoonPunchIn->format('H:i') < '13:00') {
                    $abehavior = 'Early';
                } elseif ($afternoonPunchIn->format('H:i') >= '13:00') {
                    $abehavior = 'Regular';
                } elseif ($afternoonPunchIn->format('H:i') > '13:30') {
                    $abehavior = 'Late';
                } else {
                    // Handle unexpected cases if necessary
                    return response()->json(['error' => 'Invalid punch-in time'], 400);
                ...
                Attendance::create([
                    'user_id' => $request->employee,
                    'date' => $request->date,
                    'punched_in' => $afternoonPunchIn,
                    'punched_out' => $afternoonPunchOut,
                    'behavior' => $abehavior,
                    'type' => 'Manual',
                    'total_hours' => $total_hours,
                    'entry' => 'Double',
    ...
    public function approveleave(Leave $leave) {
        if ($leave) {
            ...
            $totalLeaves = Leave::where('user_id', '=', $leave->user_id)->where('status', '=', 'Approved')
            ->whereYear('updated_at', '=', Carbon::now()->year)
            ->get();
    ...
    public function disapproveleave(Leave $leave){
        if ($leave) {
            $leave->status = 'Disapproved';
            $leave->save();  
        }
    ...
    public function employeedetails($id) {
        ...
        $user = Auth::user();
        return view('admin.employees.employeedetails', compact('employee', 'user'));
    }
    ...
    public function employeeleaveallowance($id) {
        ...
        $user = Auth::user();
        return view('admin.employees.details.employeeleaveallowance', compact('employee', 'user'));
    }
    ...
    public function employeeattendance($id) {
        ...
        $monthSearch = 'false';
        return view('admin.employees.details.employeeattendance', compact('employee', 'user', 'getAttendance', 'totalActiveHours', 'monthSearch'));
    }
    ...
    public function employeeleave($id) {
        ...
        $user = Auth::user();
        return view('admin.employees.details.employeeleave', compact('employee', 'user', 'leaves'));
    }
    ...
    public function leaveallowance(){
        ...
        return view('admin.jobdesk.leaveallowance',compact('user', 'sss', 'pagibig', 'philhealth'));
    }
    ...
    public function employeedocuments($id) {
        ...
        return view('admin.employees.details.employeedocuments', compact('user', 'documents', 'employee'));
    }
    ...
    public function employeeassets($id) {
        ...
        return view('admin.employees.details.employeeassets', compact('user', 'documents', 'employee'));
    }
    ...
    public function employeejobhistory($id) {
        ...
        return view('admin.employees.details.employeejobhistory', compact('user', 'employee', 'departments', 'shifts', 'designations', 'estatus'));
    }
    ...
    public function employeesalaryoverview($id) {
        ...
        return view('admin.employees.details.employeesalaryoverview', compact('user', 'employee'));
    }
    ...
    public function employeepayslip($id) {
        ...
        return view('admin.employees.details.employeepayslip', compact('user', 'employee', 'payslip'));
    }
    ...
    public function employeeaddressdetails($id) {
        ...
        return view('admin.employees.details.employeeaddress', compact('user', 'employee'));
    }
    ...
    public function employeeupdateaccount($id) {
        ...
        return view('admin.employees.details.updateaccount', compact('user', 'employee'));
    }
    ...
    public function employeesaveupdateaccount(Request $request, $id) {
        ...
        return redirect()->route('employeeupdateaccount', $employee->id)
    ...
    public function updateUser(Request $request, $id)
    {
        ...
        if ((float)$request->salary !== (float)$employee->salary) {
            $employee->salary = $request->salary;
        }

        if ((int)$request->role !== (int)$employee->role) {
            $employee->role = $request->role;
        }

        // Department / designation / employment status
        if ($request->filled('department') &&
            (int)$request->department !== (int)$employee->department_id) {
            $employee->department_id = $request->department;
        }

        if ($request->filled('designation') &&
            (int)$request->designation !== (int)$employee->designation_id) {
            $employee->designation_id = $request->designation;
        }

        if ($request->filled('status') &&
            (int)$request->status !== (int)$employee->employment_id) {
            $employee->employment_id = $request->status;
        }
